...
#define A_HEADER "~ F S UID     SZ        TTY   CMD\n"
#define A_FORMAT "%3o %c %3d %6d %10s %s\n"
...
char *argv[];
{
  ...
  printf("%s", opt_long ? L_HEADER : (opt_alt ? A_HEADER : S_HEADER));
  for (i = -nr_tasks; i < nr_procs; i++) {
	if (pstat(i, &buf) != -1 &&
		...
		else if (opt_alt) printf(A_FORMAT,
				buf.ps_flags, buf.ps_state,
				buf.ps_euid,
				off_to_k((buf.ps_tsize +
				buf.ps_stack - buf.ps_data + 
